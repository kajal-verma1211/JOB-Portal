...
// updating the data
       ...
       if (skills) {
        user.profile.skills = skills.split(",").map(skill => skill.trim());  // convert string to array
    } else {
        user.profile.skills = user.profile.skills || [];  // ensure it's always an array
    }

        //resume
        if(cloudResponse){
            user.profile.resume=cloudResponse.secure_url//save to cloudanary url
